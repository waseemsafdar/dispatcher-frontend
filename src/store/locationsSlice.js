...
export const saveLocation = createAsyncThunk('locations/saveLocation', async (locationData) => {
  const response = await axios.post('http://18.222.6.154:5000/locations/', locationData);
  return response.data;
});

export const getLocations = createAsyncThunk('locations/getAll', async () => {
  const response = await axios.get('http://18.222.6.154:5000/locations');
  return response.data;
});

export const fetchLocationById = createAsyncThunk('locations/fetchById', async (id) => { 
  const response = await axios.get(`http://18.222.6.154:5000/locations/${id}`); 
  return response.data; 
});
  ...
  async ({ id, values }) => {
    try {
      const response = await axios.put(
        `http://18.222.6.154:5000/locations/${id}`, // URL with ID
